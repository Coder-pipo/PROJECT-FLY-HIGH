...
{
	$query = "SELECT * FROM msc   WHERE college_status = '1'";
...
?>
			<div class="col-sm-4 col-lg-3 col-md-4">
				<div style="position:relative; border:1px solid #ccc; border-radius:5px; padding:16px; margin-bottom:16px; height:550px;">
					
					
					<img src="<?php echo "msc/". $row['image']?>" alt="" class="img-responsive" >
					...
					<a href="javascript:void(0)" onclick="wishlist_manage('<?php echo $row['collegename']?>','<?php echo $row['officialwebsite']?>','<?php echo "msc/".$row['image']?>','<?php echo $row['city']?>','add')"><button type="button" style="position:absolute; bottom:10px ;" class="btn btn-outline-info btn-sm">Add to Wishlist</button></a>
