...
<?php
	...
	{
		$query = "SELECT * FROM bca WHERE college_status = '1'";
...
?>
	...
	<img src="<?php echo "bca/".$row['image']?>" alt="" class="img-responsive" >
	...
	<h8 style="text-align:right;" class="text-light"><?php echo "Approved By: ". $row['approvedby']?></h8><br>
		...
		<a href="javascript:void(0)" onclick="wishlist_manage('<?php echo $row['collegename']?>','<?php echo $row['officialwebsite']?>','<?php echo "bca/".$row['image']?>','<?php echo $row['city']?>','add')"><button type="button" style="position:absolute; bottom:10px ;" class="btn btn-outline-info btn-sm">Add to Wishlist</button></a>
